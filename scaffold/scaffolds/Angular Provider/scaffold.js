...
/**
 * Creates a new Angular provider with specs. Registers
 * the new component to the given module.
 *
    ...
    return inquirer.prompt([
        {
            type: 'string',
            name: 'name',
            message: 'What is the name of this new Angular Provider?',
        ...
        let file = path.resolve(answers.module, 'providers/index.js'),
            contents = fs.readFileSync(file).toString('utf8');

        contents += `\nimport ${answers.name} from './${dirname}';`;
        contents += `\nexport {${answers.name} as ${answers.name}};`;
        contents += `\n${_.snakeCase(path.basename(answers.module)).toUpperCase()}_PROVIDERS.push(${answers.name});`;
        ...
        assemble
            ...
            .pipe(rename(path => {
                path.basename = path.basename.replace('Provider', answers.name);
                if (path.extname === '.js') {
                    path.extname = config.extensions.scripts;
                }
            }))
            .pipe(assemble.dest(path.resolve(answers.module, 'providers', dirname)));
    });
};
